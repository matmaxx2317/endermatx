...
### Tipp-Klatsch in morning reports

`do_generate_summary` also optionally appends a "## Tipp-Klatsch" section — friendly, gossip-style commentary about the **opponents' tip performance**, generated purely from imported `WmtOpponentTip` data (no NewsAPI involved). `_compute_opponent_tip_breakdown(db, target)` tallies each player's tendency-correct tips per round (MD1/MD2/MD3/knockout stages) plus overall and "yesterday" totals, up to and including the target date; `_format_tip_breakdown_text` renders it as a compact table (e.g. `Player_01: MD1 3/3, MD2 1/3 — gesamt 4/6, gestern 1/3`); `_generate_tip_gossip` (Claude API, `claude-haiku-4-5-20251001`) turns that into 2-4 short, warm, "loving mockery among friends" remarks about standout patterns — who's on a hot streak, who's cratered, who's wildly consistent — referencing both the latest day and the tournament-wide trend. Only runs if `ANTHROPIC_API_KEY` is set and at least one opponent tip exists for a finished match; fails silently like the regular Gossip section.
...
6. The same Tippübersicht screenshot also shows each player's current **Pos** (rank) and **P** (points) in the leaderboard header/footer — Claude extracts those too and POSTs them to `/api/wmt/rankings/import` as `{snapshots: [{date, player_name, rank, points, snapshot_time?}, ...]}`, where `date` is the calendar day the screenshot represents (not the import date — historical catch-up imports are fine as long as the represented date is correct). The endpoint upserts a `WmtRankingSnapshot` row per `(date, player_name, snapshot_time)`. `snapshot_time` is optional and normally omitted (one snapshot per day, representing end-of-day); set it (an ISO UTC datetime) only when importing an **additional intermediate** snapshot for a day that already has one — e.g. the state right after a specific match's result was revealed, before later matches that day finished. There is no need to reason about "which game this snapshot should line up with" when omitting `snapshot_time` — the chart positions it automatically via `captured_at` (see below).

### Rank evolution chart (`WmtRankingSnapshot`)

Kicktipp has no public export and its exact point formula (exact-score / goal-difference bonuses) isn't published, so the app doesn't try to recompute the leaderboard — instead it captures the **official** Pos/P values straight from the daily Tippübersicht screenshot (see step 6 above) and stores them as `WmtRankingSnapshot` rows. Because historical ranks can't be reconstructed retroactively, the chart is only as complete as the daily screenshot habit — there is no backfill. The **Konkurrenz** tab renders a `RankingChart` (custom inline SVG, no charting library — `polyline` per player, axis gridlines/labels, a cycling colour palette) plotting rank (y-axis, inverted, rank 1 at top) against an x-axis of "games played so far" (0 to the total match count) — each snapshot is positioned by counting how many WMT matches had finished by its effective timestamp, so rest days don't stretch the timeline. The effective timestamp is `snapshot_time` if set, otherwise the earlier of `captured_at` (when the import actually happened, set automatically by the server) and end-of-`date` — this way a same-day import made before that day's later matches finish is positioned before those matches automatically, without the importing session needing to know about per-match positioning. A faint vertical line + rotated "HOME-AWAY" label marks each match's position, evenly spaced in chronological order. The chart has a horizontal zoom slider and a per-player legend that toggles each line on/off (with "Alle"/"Keine" select-all buttons) — only selected players' lines render.

**Schema note:** `wmt_ranking_snapshots.snapshot_time` (nullable `TIMESTAMP`) must be added manually to the Railway Postgres table (no migration tool — see "Database" above) before importing snapshots with `snapshot_time` set. `captured_at` already existed prior to this and requires no schema change.
